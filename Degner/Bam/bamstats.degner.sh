...
find '/proj/b2012046/rani/analysis/degner' -name 'accepted_hits.sorted.nodup.bam' -exec sh -c "samtools flagstat {} >{}.samtools.flagstat.out" \; &
find '/proj/b2012046/rani/analysis/degner' -name '*.samtools.flagstat.out' | xargs -I% awk 'NR == 1 {print FILENAME, $1;}' % >nmapped.reads
...
find '/proj/b2012046/rani/analysis/degner/mergebam' -name '*.merged.bam' | awk -F'/' -v ods="$ods" -v projid="$projid" -v em="$email" -v prg="$prg" '{print "perl", prg, $0, $8, ods, projid, em;}' >bedtoolscov.sh

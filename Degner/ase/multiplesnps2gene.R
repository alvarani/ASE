...
if(sys == 'local'){
  ase.datadir = '/proj/b2012046/rani/analysis/degner/ase'
  annovar.datadir = '/proj/b2012046/rani/analysis/degner/annovar'  
}
...
main <- function(){
  ...
  length(unique(unlist(lapply(ase.res, '[[', 'site')))) #72377
  ...
  vars.list = lapply(ase.res, function(sample.vars, vars.annot){vars = merge(sample.vars, vars.annot, by.x = 'site', by.y = 'chrpos'); return(vars)}, vars.annot = varcalls)
  ...
  print(a)
  print(b)
  length(unique(unlist(lapply(ase.res, rownames)))) #72377
  nrow(varcalls) #72428
  ...
  length(unique(unlist(lapply(genes2asefrac.filt, rownames)))) #625

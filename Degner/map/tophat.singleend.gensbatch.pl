...
my ($fastqprefix, $outdir, $threads, $gtf, $ref,$ods, $projid, $email, $time, $readlen) = @ARGV;

#set vars
my $fastqfile_1 = $fastqprefix . '1';
...
#set sbatch file
my $sbatch_file = catfile($ods, $fastqprefix_base . '.tophat.sbatch');
$sbatch_file =~ s/\.\./\./;

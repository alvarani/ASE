...
dbsnp.file = file.path(dbsnp.datadir, 'dbsnp.snp135.common.txt')


#Res
bc.file = file.path(basecount.datadir, 'bc.RData')
bc.dbsnp.file = file.path(dbsnp.datadir, 'bc.dbsnp.RData')
ase.res.file = file.path(resdir, 'ase.res.RData')
...
main <- function(){
  ...
  dbsnp.filt = lapply(bc.filt, function(jsample.vars, dbsnp.vars){merge(jsample.vars, dbsnp.vars, by = 'site')}, dbsnp.vars = dbsnp.vars)

  #total number of vars of dbsnp
  length(unique(unlist(lapply(dbsnp.filt, '[[', 'site')))) #77969

  #Dump
  save( dbsnp.filt, file = bc.dbsnp.file)
  ...
  length(unique(unlist(lapply(sig.vars, '[[', 'site')))) #39044

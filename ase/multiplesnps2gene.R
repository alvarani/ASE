...
main <- function(){
  ...
  length(unique(unlist(lapply(ase.res, '[[', 'site')))) #112756
  ...
  vars.list = lapply(ase.res, function(sample.vars, vars.annot){vars = merge(sample.vars, vars.annot, by.x = 'row.names', by.y = 'chrpos'); rownames(vars) = vars[, 'Row.names']; vars = vars[, setdiff(colnames(vars), 'Row.names')]; return(vars)}, vars.annot = varcalls)

  #Check that same number of vars before and after annot
  a = unlist(lapply(ase.res, nrow))
  b = unlist(lapply(vars.list, nrow))
  length(unique(unlist(lapply(ase.res, rownames)))) #112756
  nrow(varcalls) #112901
  #OK!
  ...
  length(unique(unlist(lapply(genes2asefrac.filt, rownames)))) #3424

...
sbatchdir='/proj/b2012046/rani/scripts/gsnap/unmapped'
bamfile='/proj/b2012046/rani/analysis/gsnap/bamsort/'
cd $sbatchdir
find $bamfile -name '*.uniqmapped.bam' | awk -F'/' -v execdir=$execdir -v sbatchdir=$sbatchdir -v projid=$projid -v email=$email '{print "perl", execdir"/rmdups.pl", $0, $NF, sbatchdir, projid, email;}' >rmdups.sh
sh rmdups.sh
